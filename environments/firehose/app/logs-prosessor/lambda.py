...
def transformLogEvent(log_event):
    ...
    d = { 'timestamp': log_event['timestamp'], 'message': log_event['message'] }
    return json.dumps(d) + '\n'
...
def processRecords(records):
    for r in records:
        ...
        if data['messageType'] == 'CONTROL_MESSAGE':
            ...
            jst_time = datetime.datetime.fromtimestamp(data['logEvents'][0]['timestamp'] // 1000, tz=jst)
            print('jst_time')
            print(jst_time)
            partition_keys = {
                'logGroup': data['logGroup'].lstrip('/').replace('/','-'),
                'logStream': data['logStream'].lstrip('/').replace('/','-'),
                'year': jst_time.strftime('%Y'),
                'month': jst_time.strftime('%m'),
                'day': jst_time.strftime('%d'),
                'hour': jst_time.strftime('%H'),
                'minute': jst_time.strftime('%M')
            }
